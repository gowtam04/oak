...
The agent loop is **provider-agnostic** behind the `LLMProvider` seam (`src/agent/providers/`). The registry is **per-model, not per-provider** — `ModelKey = "grok-4.3" | "claude-sonnet-5" | "claude-sonnet-4.6" | "gpt-5.5"` (multiple models per provider are supported; the old ambiguous `"claude"` key was retired 2026-07 and survives only as a legacy `MODEL_PRICING` entry so historical `turn_record` rows still cost-estimate). Four models plug in across three provider adapters: **xAI Grok 4.3** — the primary/default, a *native* adapter on xAI's **Responses API** (`grok-provider.ts`); **Claude Sonnet 5** and **Claude Sonnet 4.6** (`anthropic-provider.ts`); and **OpenAI GPT-5.5** (`openai-compatible-provider.ts`, the Chat Completions shim). The split:

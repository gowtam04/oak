...
The agent loop is **provider-agnostic** behind the `LLMProvider` seam (`src/agent/providers/`). The registry is **per-model, not per-provider** — `ModelKey = "grok-4.3" | "grok-4.5" | "claude-sonnet-5" | "claude-sonnet-4.6" | "gpt-5.5"` (multiple models per provider are supported; the old ambiguous `"claude"` key was retired 2026-07 and survives only as a legacy `MODEL_PRICING` entry so historical `turn_record` rows still cost-estimate). Five models plug in across three provider adapters: **xAI Grok 4.3** — the primary/default, and **Grok 4.5** (admin-selectable), both on the *native* adapter on xAI's **Responses API** (`grok-provider.ts`); **Claude Sonnet 5** and **Claude Sonnet 4.6** (`anthropic-provider.ts`); and **OpenAI GPT-5.5** (`openai-compatible-provider.ts`, the Chat Completions shim). The split:

- **`models.ts`** — CLIENT-SAFE registry (no SDK / `@/env`): `ModelKey`, `MODELS` (display order — **Grok first**), `DEFAULT_MODEL_KEY = "grok-4.3"`, plus `modelLabel`/`isModelKey`. Adding a model is one line here + one `factory.ts` entry + one `MODEL_PRICING` entry (`src/server/admin/pricing.ts`). (There is still **no per-turn/end-user model picker** — but there IS an operator-facing one: the admin **Settings** tab, see below.)

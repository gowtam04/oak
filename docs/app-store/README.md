...
**Store name:** `Oak – AI Coach` · **Subtitle:** `Team Builder & Calculator`. The bare name "Oak" was taken in the App Store, so a descriptor was appended — this changes only the App Store *display name*, not the bundle id (`ai.gowtam.oak`), the `oak.gowtam.ai` domain, or the in-app "Oak" assistant persona. Home screen stays `Oak`. Live 1.0.2 still shows `Oak – AI Battle Coach` until 1.2 is approved.
...
- [`screenshots.md`](./screenshots.md) — 6-frame screenshot production guide. The 1.2 set currently on App Store Connect is still Signal; regenerate in Enamel with [`enamel-screenshots-agent-prompt.md`](./enamel-screenshots-agent-prompt.md).

## Trademark policy applied throughout

- **App Name, Subtitle, Keywords:** never "Pokémon" / "Pokédex".
...
- Subtitle stays `Team Builder & Calculator` rather than saying Pokémon Champions (CF-UI-BR-1 vs this policy — policy wins).
...
- The Keywords string in `ios.md`/`aso-keywords.md` must match (1.2 dropped `ev`/`iv`/`tera`, added `mega`/`usage`).

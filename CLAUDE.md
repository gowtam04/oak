...
- The active model is **server-controlled** (`AgentContext.model`, from the validated request body's `model` key) — like `mode`, never an LLM-visible tool input. Per-provider prompt styles live in `src/agent/prompts/style-{claude,openai,grok}.ts`, selected by `provider.kind` in `buildSystemSegments`.

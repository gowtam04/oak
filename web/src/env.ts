...
const EnvSchema = z
  .object({
    ...
    // treated as absent, like RESEND_API_KEY below. The active model is chosen in
    // the admin Settings panel (persisted in `app_setting`), so selecting an
    // alternate provider just requires its key to be present.
    ANTHROPIC_API_KEY: z.preprocess(
      emptyToUndefined,
      z.string().min(1).optional(),
    ),
    // Used ONLY by the eval judge (web/eval/judge.ts) and as the AnthropicProvider
    // constructor default — the agent registry hardcodes API model ids in the
    // provider factory (`MODEL_CONFIG`), so it does NOT read this.

 ...
 *   - truncated candidates → CandidateTable Browse (`candidate-table-browse`)
 *     opens the viewer (not a follow-up turn)
...
describe("AnswerCard — follow-up affordances gate on `disabled` (U2)", () => {
  it("fires onFollowUp for every affordance when not disabled", () => {
    ...
    // Browse all opens the viewer — it is not a follow-up turn.
    expect(screen.getByTestId("candidate-table-browse")).toBeInTheDocument();
  });
  ...
  it("disables every follow-up affordance and ignores clicks while streaming", () => {
    ...
    const browse = screen.getByTestId("candidate-table-browse");
  ...
  it("previews truncated candidates and Browses instead of expanding in the card (T2)", () => {
    const onFollowUp = vi.fn();

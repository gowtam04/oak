 ...
 *   - Champions-first (ADR-4): every index row is format="champions". Extra
 *     gen-7 / National Dex / wiki / encounter partitions are not seeded.
 *
...
export const REFERENCE_CACHE_SEED: ReferenceCacheSeed[] = [
  ...
  // --- T7 get_evolution_chain: a Champions evolution HIT ----------------------
  {
...
// ---------------------------------------------------------------------------
// Seeder — Champions partition only (ADR-4, CF-DATA-BR-3)
// ---------------------------------------------------------------------------

/**
 * Insert the Champions fixture into a freshly-migrated Postgres schema via
 * Drizzle. Idempotent on a fresh schema; the caller must supply an empty,
 * migrated handle (typically `createPgSchema({ seed: "tools" })` does this).
 */
export async function seedToolsFixture(db: ToolsFixtureDb): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    await tx
      .insert(pokemon)
      .values(POKEMON_SEED.map((p) => ({ ...p, format: CHAMPIONS })));
    await tx.insert(learnset).values(LEARNSET_SEED);
    await tx
      .insert(searchable_names)
      .values(SEARCHABLE_NAMES_SEED.map((n) => ({ ...n, format: CHAMPIONS })));

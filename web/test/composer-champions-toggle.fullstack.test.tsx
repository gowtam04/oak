 ...
 * controls row (moved out of the header) and defaults to ON / Champions.
 *
 * Renders the REAL <Home/> as a guest with a single stubbed `fetch`, and drives:
 *   - a fresh load shows the toggle in the composer, aria-checked="true"
 *     (the Champions default),
 *   - sending a message posts `champions_mode: true`,
 *   - clicking the toggle flips it off and the NEXT send posts
 *     `champions_mode: false` (and the choice persists to localStorage).
 *
...
describe("Composer Champions toggle", () => {
  it("lives in the composer and defaults to ON (Champions) on a fresh load", async () => {
    render(<Home />);
  ...
  it("sends champions_mode:true by default (never toggled)", async () => {
    render(<Home />);
    await screen.findByTestId("composer");
  ...
  it("toggling off before the first message scopes the turn to Standard", async () => {
    render(<Home />);
    await screen.findByTestId("composer");

    // Flip the toggle off — it dims and persists the choice.
    const toggle = screen.getByTestId("champions-toggle");
    await act(async () => {
      fireEvent.click(toggle);
    });
    expect(toggle).toHaveAttribute("aria-checked", "false");
    expect(localStorage.getItem(CHAMPIONS_STORAGE_KEY)).toBe("false");

...
/// `Format` widened from 2 to 6 known cases plus a tolerant `.unknown(String)`
/// fallback (Phase 1 of the iOS↔web parity plan) — this is a correctness fix, not
/// just added coverage: before the widening, a web-created gen-scoped
/// conversation/team (`gen-5`…`gen-8`) would fail to decode at all, breaking the
...
struct FormatDecodingTests {
  ...
  /// Each of the six known wire strings (`web/src/data/formats.ts` `FORMATS`)
  ...
  /// A format string outside the known six decodes to `.unknown(raw)` rather
  ...
  /// The real-world case this all guards: a conversations list mixing all six
  ...
  func conversationsListDecodesAllSixFormatsPlusUnknownTolerantly() throws {
    ...
    #expect(env.conversations[6].format == .unknown("gen-99-mystery"))
  }
